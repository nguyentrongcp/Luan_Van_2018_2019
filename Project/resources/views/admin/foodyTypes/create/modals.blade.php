...
@foreach($foodyTypes as $foodyType)
    <div class="modal fade" id="delete-foody-type-modal-{{$foodyType->id}}" tabindex="-1" role="dialog" aria-labelledby="delete" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <p class="text-black-50">Các dữ liệu liên quan đến <strong> {{$foodyType->name}} </strong>sẽ bị xóa hoàn toàn. Bạn cần suy nghĩ trước khi xóa?</p>
                </div>
                <div class="modal-footer">
                    <form action="{{route('add_new.destroy',[0])}}" method="post">
                        {{ method_field('DELETE') }}
                        {{ csrf_field() }}
                        <input type="hidden" name="foody-type-id" value="{{ $foodyType->id }}">
                        <button type="button" class="btn btn-secondary btn-round" data-dismiss="modal">ĐÓNG</button>
