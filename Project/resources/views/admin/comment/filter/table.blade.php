<div class="tables">
    <table class="table table-bordered">
        ...
        <tbody>
        @foreach($showFilters as $stt => $showFilter)
            <tr>
                <td class="text-left td-prot th-stt">
                    <div class="form-check">
                        <label class="form-check-label">
                            <input class="form-check-input" name="comment-id[]" value="{{$showFilter->id}}" type="checkbox">
                            <span class="form-check-sign"></span>
                        </label>
                    </div>
                </td>
                <td class="text-center td-prot th-stt">{{$stt + 1}}</td>
                <td class="td-prot">{{$showFilter->title}}</td>
                @php
                    $idFoodys = App\Foody::where('id',$showFilter->foody_id)->get();
                ...
                <td class="td-prot text-center">
                    @if($showFilter->is_approved == false)
                ...
                <td class="td-actions text-center td-prot">
                    <a rel="tooltip" data-placement="bottom" title="Xem chi tiết"
                       ...
                       href="{{route('comments.show',[$showFilter->foody_id])}}">
                        <i class="fa fa-eye"></i>
                    </a>
                </td>
                <form action="{{route('admin.approved',[$showFilter->id])}}" method="post">
                    {{ csrf_field() }}
                    <td class="th-stt text-center td-prot">
                        <button type="submit" rel="tooltip" data-placement="bottom" title="Duyệt"
                                class="btn btn-success btn-sm btn-round btn-icon {{$showFilter->is_approved == true ?'disabled':''}}"
